...
func getSNI(conn net.Conn) (string, error) {
	...
	peeker := &peekingConn{Conn: conn, reader: bufio.NewReader(conn)}
...
func runServer(listenAddr, secret string) {
	...
	if err != nil {log.Fatalf("初始化 TLS 配置失败: %v", err)}
	listener, err := tls.Listen("tcp", listenAddr, tlsConfig)
	if err != nil {log.Fatalf("无法监听 TLS 地址 %s: %v", listenAddr, err)}
	...
	for {
		...
		if err != nil {log.Printf("接受 TLS 连接失败: %v", err); continue}
...
func handleClientSession(tunnelConn net.Conn) {
	...
	smuxConfig.KeepAliveInterval = 10 * time.Second // 每10秒发送一次心跳
	smuxConfig.KeepAliveTimeout = 30 * time.Second  // 30秒无响应则认为连接断开
...
func runClientSession(tunnelConn net.Conn, localTargetAddr string, remotePort int) {
	...
	for {
		...
		if err != nil {
			if err == io.EOF || strings.Contains(err.Error(), "use of closed network connection") || strings.Contains(err.Error(), "broken pipe") || strings.Contains(err.Error(), "timeout") {
				log.Println("隧道会话已关闭或超时。")
			} else {
